...
function weekGames(key, withHidden, withOpp){
  ...
  schedMissing(Object.values(out), key).forEach(x => { out[x.id] = x; });   // the rest of the week, from the schedule file
  ...
  byPair.forEach(xs => {
    ...
    const best = xs.filter(x => plays(x) > 0).sort((a, b) => plays(b) - plays(a) || (b.updated || 0) - (a.updated || 0))[0];
    const hasScore = xs.some(x => x.kind === 'score');
    xs.forEach(x => {
      if (best ? x !== best : hasScore && x.kind !== 'score') delete out[x.id];
    });
  });
  if (!withHidden) Object.values(out).forEach(x => { if (isHidden(x)) delete out[x.id]; });
...
function scoreCard(x){
  ...
  const status = m.fin && gameDay(x).getDay() !== 5 ? `${m.status} - ${dayName}` : m.status;
  const inner = `<div class="sc-st">${esc(status)}${m.men === 8 ? '<span class="sc-8">8-man</span>' : ''}</div>${row('A')}${row('H')}`;
...
document.addEventListener('click', e => {
  ...
  // The scorer's own pages, and the admin wherever they are: the score window works the same in both.
  if (!ui.viewer || ui.admin){
    ...
    if (ks && !ui.viewer){ ui.fromSched = ks.dataset.qsStats; return openDialog('new'); }
