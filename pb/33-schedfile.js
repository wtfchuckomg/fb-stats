...
function schedStubs(){
  ...
  sched.byPair = new Map(sched.stubs.map(x => [schedPair(x), x]));
  return sched.stubs;
}
// The file's final for a game the site is still showing at 0-0. The opponents' schedules were seeded once, when
// those games hadn't been played, and each one is a saved entry that outranks the schedule file — so without this
// a result sits in the file while the team page still reads "Fri 9/18". A score someone typed, a tracked game and
...
function schedFinal(x){
  ...
  // Whichever side the file has at home may be the other way round from ours: go by the school, not the side.
  const home = kpAlias(r.teams.H.name) === kpAlias(x.teams.H.name);
  return {A:home ? r.A : r.H, H:home ? r.H : r.A};
}
