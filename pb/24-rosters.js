...
function shareRoster(t){
  ...
  if (!Object.keys(t.roster || {}).length || !sharedRosters.list) return;   // (not before the saved copies are in)
  // Adds to your copy, never shrinks it: a game set up with half a roster mustn't wipe out the full one saved
  // for the school (a roster is cut down only on the school's own page).
  const id = `roster-${teamKey(t.name)}-${sync.user.uid}`, mine = (sharedRosters.list || []).find(r => r.id === id);
  const roster = Object.assign({}, mine && mine.roster, t.roster), n = Object.keys(roster).length;
  const {fsM, fsdb} = sync.api, r = {name:t.name, roster, count:n, updated:Date.now()};
  fsM.setDoc(fsM.doc(fsdb, 'pressbox', id),
