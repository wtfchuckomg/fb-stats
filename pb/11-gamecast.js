...
function cardTeamTable(){
  return `<section class="card"><div class="card-hd"><h2 class="card-title">Stat Details</h2></div><div class="tbl-wrap"><table class="st">
    ...
    <tbody>${teamRows().map(([l, f]) => `<tr><td>${l}</td><td>${f('A')}</td><td>${f('H')}</td></tr>`).join('')}</tbody></table></div>
    <p class="hint" style="margin-top:10px">Sacks count against rushing, the usual high-school and college stat convention.</p></section>`;
}
function cardDrives(){
