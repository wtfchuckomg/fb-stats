...
const allGames = {list:null, idx:null, err:'', unsub:null};
...
function indexGames(){
  ...
  (allGames.list || []).forEach(x => {
    ...
    const stats = x.kind !== 'score' && ((x.plays && x.plays.length) || !!x.box), wk = gameWeek(x);
    ['A', 'H'].forEach(side => {
...
function teamPageHtml(nameIn){
  ...
  const sched = rows.map(({x, side, opp, wk, stats}) => {
    ...
    const form = open ? `<tr class="tp-edit-row"><td colspan="4"><div class="tp-score-form">
...
document.addEventListener('click', e => {
  ...
  if (sc){ tpage.editing = {id:sc.dataset.tpScore, side:sc.dataset.side}; tpage.sdraft = null; return renderTeamPage(); }
  if (t.closest('[data-tp-score-cancel]')){ tpage.editing = null; tpage.sdraft = null; return renderTeamPage(); }
