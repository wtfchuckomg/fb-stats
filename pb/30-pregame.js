...
function predict(A, H){
  const side = n => {
    ...
    const sos = typeof sosOf === 'function' ? sosOf(n) : null;
  ...
  const parts = [
    ...
    // Who they played, not just how they did: the ratings' view of every school on the card so far. This line
    // was worth far too little. Tested against this season's 457 results — with each game taken back out of
    // its own two teams' records first, so nothing is judged on knowing its own outcome — the predictor picked
    // 75.1% of games straight up and missed the margin by 23.7 points at the old weight, and 80.1% missing by
    // 22.5 at this one. A record counts for more alongside it, since it is now read against the schedule
    // behind it. Two games still say little about a schedule, so it grows as they are played.
    ['Schedule faced', a.sos && h.sos && a.sos.faced != null && h.sos.faced != null
      ? Math.max(-16, Math.min(16, 1.2 * (h.sos.faced - a.sos.faced) * (n => n / (n + 2))(Math.min(a.s.gp, h.s.gp)))) : 0],
    ['Record', 16 * (h.wp - a.wp)],
...
/* ---------- strength of schedule ----------
   The line (ourLine) already takes the schedule into account: the ratings are fitted to every result at once,
   so beating a good team moves a school further than beating a poor one. The predictor's record line did not —
   3-0 counted the same however it was earned. This is what fixes that, and what the SOS numbers on the pages
   are drawn from. Ratings are in points, and a game's expected margin is simply the difference between them. */
const SOS_SIGMA = 14;                        // how often the better team actually wins, in points
let sosMemo = {}, sosStamp = '';
function sosOf(name){
  const k = canonSchool(name); if (!k || !pre.rat || !sched.rows) return null;
  // Games and schedules arrive one after another: an answer worked out before they landed must not be kept.
  const stamp = `${(allGames.list || []).length}|${sched.rows.length}|${(logoLib.list || []).length}`;
  if (stamp !== sosStamp){ sosMemo = {}; sosStamp = stamp; }
  if (sosMemo[k]) return sosMemo[k];
  schedStubs();
  const full = [], faced = [];
  let exp = 0, act = 0, gp = 0;
  // The whole 2026 card, from the schedule file: who they play, whatever has been played so far.
  sched.stubs.forEach(x => {
    const side = canonSchool(x.teams.A.name) === k ? 'A' : canonSchool(x.teams.H.name) === k ? 'H' : null;
    if (!side) return;
    const opp = ratingOf(x.teams[side === 'A' ? 'H' : 'A'].name);
    if (opp) full.push(opp.rating);
  });
  // The games actually played, and how likely each one was to be won by a team of this quality.
  const me = ratingOf(name);
  schoolRows(name).forEach(({x, side, opp}) => {
    const f = finalOf(x); if (!f.fin || !f.score) return;
    const other = ratingOf(x.teams[opp].name); if (!other || !me) return;
    const hfa = (pre.rat.groups[me.group] || {hfa:1.5}).hfa;
    const edge = me.rating - other.rating + (side === 'H' ? hfa : -hfa);
    exp += 1 / (1 + Math.exp(-edge / SOS_SIGMA));
    act += f.score[side] > f.score[opp] ? 1 : f.score[side] === f.score[opp] ? .5 : 0;
    faced.push(other.rating); gp++;
  });
  const mean = a => a.length ? a.reduce((t, v) => t + v, 0) / a.length : null;
  const out = {full:mean(full), faced:mean(faced), n:full.length, gp, exp, act,
    over:gp ? (act - exp) / gp : 0};   // wins above what the schedule asked for, per game
  sosMemo[k] = out;
  return out;
}
// Where a school's schedule ranks in its own class, the way a strength-of-schedule table reads.
function sosRank(name){
  const info = schoolInfo(name), cls = info && info[0]; if (!cls) return null;
  const peers = (logoLib.list || []).filter(t => { const i = schoolInfo(t.name); return i && i[0] === cls; });
  const rows = peers.map(t => ({name:t.name, sos:(sosOf(t.name) || {}).full})).filter(r => r.sos != null)
    .sort((a, b) => b.sos - a.sos);
  const at = rows.findIndex(r => canonSchool(r.name) === canonSchool(name));
  return at < 0 ? null : {rank:at + 1, of:rows.length, cls, sos:rows[at].sos};
}
...
function sosNote(A, H, P){
  ...
  const row = (n, o) => `<tr><td>${esc(shortName(n))}</td><td class="num">${o.gp ? num(o.faced) : '\u2014'}</td>`
