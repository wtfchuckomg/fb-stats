...
async function startViewer(id){
  ...
  $('#sample').hidden = true;
  $('#view').innerHTML = '<div class="panel"><div class="empty">Loading the live game…</div></div>';
  let fsM, fsdb, asAdmin = false, noted = false;
  try {
    const base = `https://www.gstatic.com/firebasejs/${FIREBASE_VERSION}/firebase-`;
  ...
  fsM.onSnapshot(fsM.doc(fsdb, 'pressbox', id), snap => {
    ...
    if (d.public && location.hostname === 'stats.kansasmediarankings.com' && !location.pathname.startsWith('/g/') && !startViewer.checked){
