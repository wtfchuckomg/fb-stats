...
let schTap = null;                      // the suggestion a finger is down on
const schBoxOf = opt => { const b = opt.closest && opt.closest('.sch-sug'); return b ? document.getElementById(String(b.id).replace(/-sug$/, '')) : null; };
...
// The box losing focus puts the list away — unless a finger is down on the list itself, which is what
// happens on a phone the moment you touch a suggestion.
document.addEventListener('focusout', e => { if (isSchoolBox(e.target) && !schTap) closeSchoolSug(e.target); });
...
document.addEventListener('pointerdown', e => {
  const sug = e.target.closest && e.target.closest('.sch-sug');
  if (!sug){    // a touch anywhere else puts an open list away, since the box may already have lost focus
    if (!isSchoolBox(e.target)) document.querySelectorAll('.sch-sug:not([hidden])')
      .forEach(b => { const el = document.getElementById(String(b.id).replace(/-sug$/, '')); if (el) closeSchoolSug(el); });
    return;
  }
  schTap = e.target.closest('.sch-opt') || null;
  // A mouse is held off the list so the box keeps focus. A finger is not: blocking it stops the list
  // scrolling, and on a phone it can swallow the tap that was meant to pick the school.
  if (e.pointerType === 'mouse') e.preventDefault();
});
// The pick is made on the release. A phone doesn't always send a click after a tap on something that just
// took the focus, and a school chosen is too important to lose to that.
document.addEventListener('pointerup', e => {
  const opt = schTap; schTap = null;
  if (!opt || (e.target.closest && e.target.closest('.sch-opt')) !== opt) return;   // slid off: not a pick
  const el = schBoxOf(opt); if (el) pickSchoolOption(el, opt);
});
document.addEventListener('pointercancel', () => { schTap = null; });   // a scroll took the finger away
if (!window.PointerEvent) document.addEventListener('click', e => {
